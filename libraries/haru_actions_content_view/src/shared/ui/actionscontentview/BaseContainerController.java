...
public class BaseContainerController implements ContainerController {
    ...
    private static final String TAG = BaseContainerController.class.getSimpleName();
...
//        Log.e(TAG, String.format("called onScroll(), isOpening: %s, enableEffects: %s", isOpening, enableEffects));

        final boolean updateEffects;
        if (enableEffects)
            updateEffects = mEffectsController.apply(factor,
                    isOpening ? EffectsController.EFFECT_OPEN : EffectsController.EFFECT_CLOSE);
        else {
